...
namespace mrs_subt_planning
{
  ...
  public:
    ...
    std::pair<int, int> firstUnfeasibleNodeInPath(const std::vector<octomap::OcTreeKey>& key_waypoints,
    ...
    findPath(const octomap::point3d& start_point, const octomap::point3d& goal_point, std::shared_ptr<octomap::OcTree> planning_octree, bool make_path_straight,
             bool apply_postprocessing, double planning_bbx_size_h, double planning_bbx_size_v, double postprocessing_safe_dist,
             int postprocessing_max_iterations, bool postprocessing_horizontal_neighbors_only, double postprocessing_z_tolerance,
             double postprocessing_path_length, int shortening_window_size, double shortening_dist, bool apply_pruning, double pruning_dist,
             bool ignore_unknown_cells_near_start = false, double box_size_for_unknown_cells_replacement = 2.0, bool remove_obsolete_points = true,
             double obsolete_points_tolerance = 0.05);
