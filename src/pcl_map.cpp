...
PCLMap::PCLMap(const std::shared_ptr<rclcpp::Node>& node, std::string node_name)
...
float PCLMap::radiusSearch(const double x, const double y, const double z, const double search_radius) {
  ...
  /* RCLCPP_WARN(m_node->get_logger()," Calling radius search, point [%.2f, %.2f, %.2f], search_radius = %.2f", x, y, z, search_radius); */
  std::vector<int>   k_indices;
  std::vector<float> k_sqr_distances;
  /* float result; */
  octree->radiusSearch(point, search_radius, k_indices, k_sqr_distances, 25);
  if (k_sqr_distances.size() > 0) {
    /* RCLCPP_INFO(m_node->get_logger(),"Point found, size = %lu", k_sqr_distances.size()); */
    /* result = *std::min_element(k_sqr_distances.begin(), k_sqr_distances.end()); */
    /* RCLCPP_INFO(m_node->get_logger(),"Smallest dist = %.2f", result); */
  ...
  /*   /1* RCLCPP_WARN(m_node->get_logger(),"result = %.2f", sqrt(k_sqr_distances[0])); *1/ */
  /*   if (sqrt(k_sqr_distances[0]) < search_radius) { */
  /*     /1* RCLCPP_WARN(m_node->get_logger(),"returning result = %.2f", sqrt(k_sqr_distances[0])); *1/ */
  /*     return sqrt(k_sqr_distances[0]); */
  /*   } */
  /* } */
  /* RCLCPP_WARN(m_node->get_logger(),"Returning -1"); */
  return -1;
}
...
void PCLMap::loadMap(const std::string &filepath, double resolution) {
  ...
  {
    RCLCPP_INFO(m_node->get_logger(),"Couldn't read file %s\n", filepath.c_str());
    return;
  }

  RCLCPP_INFO_STREAM(m_node->get_logger(),"Loaded " << cloud->width * cloud->height << " data points from " << filepath);
  octree->deleteTree();
  ...
  RCLCPP_INFO(m_node->get_logger(),"Tree depth = %d for resolution %.2f", octree->getTreeDepth(), resolution);
  pcl_cloud = cloud;
  RCLCPP_INFO(m_node->get_logger(),"Testing distance from [3.9, 20.6, 8.70] = %.2f", radiusSearch(4.8, 17.9, 8.7, 85.0));
}
...
pcl::PointCloud<pcl::PointXYZ>::Ptr PCLMap::pclVectorToPointcloud(const std::vector<pcl::PointXYZ> &points) {
  ...
  /* RCLCPP_DEBUG(m_node->get_logger(),"[POINTXYZ VECTOR TO POINTCLOUD] width, height: (%d, %d)", data->width, data->height); */
  data->is_dense = false;
...
void PCLMap::initKDTreeSearch(pcl::PointCloud<pcl::PointXYZ>::Ptr points) {
  ...
  if (points->size() > 0) {
    /* //RCLCPP_INFO(m_node->get_logger(),"initkdtree, point size = %lu",points->size()); */
    kdtree->setInputCloud(points->makeShared());
    /* //RCLCPP_INFO(m_node->get_logger(),"kdtree function end"); */
    kd_tree_initialized = true;
  }
  RCLCPP_INFO(m_node->get_logger(),"init kd tree search end");
}
...
double PCLMap::getDistanceFromNearestPoint(pcl::PointXYZ point) {
  ...
  if (kd_tree_initialized && kdtree->nearestKSearch(point, 1, indices, sqr_distances) > 0) {
    /* //RCLCPP_INFO(m_node->get_logger(),"Nearest point search: returning %.2f", sqrt(sqr_distances[0])); */
...
pcl::PointCloud<pcl::PointXYZ>::Ptr PCLMap::octomapToPointcloud(std::shared_ptr<octomap::OcTree> input_octree, std::array<octomap::point3d, 2> map_limits, bool ignore_unknown_cells) {
  ...
  if (map_limits[0].x() > map_limits[1].x() || map_limits[0].y() > map_limits[1].y() || map_limits[0].z() > map_limits[1].z()) { 
    // RCLCPP_ERROR(m_node->get_logger(),"[PCL map]: Octomap cannot be converted. Provided map limits cannot be used for definition of bounding box.");
    return nullptr;
  }

  if (!input_octree) { 
    // RCLCPP_ERROR(m_node->get_logger(),"[PCL map]: Octomap cannot be converted. Empty input octree received."); // FIXME add retunr
  }
  ...
  //RCLCPP_INFO(m_node->get_logger(),"[PCL map]: Octomap cannot be converted empty pointcloud received.");
  return nullptr;
}
