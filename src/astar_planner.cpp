                              ...
                              double max_altitude, bool debug, std::shared_ptr<mrs_lib::BatchVisualizer> batch_visualizer, const bool break_at_timeout)
{
  ...
  RCLCPP_INFO(m_node->get_logger(), "Astarplanner with resolution %.2f initialized", resolution_);
...
bool AstarPlanner::isNodeValid(const Node& n)
{
  ...
  if (isNodeInTheNeighborhood(n.key, start_.key, clearing_dist_))
  { // unknown
    return true;
...
{
  if (isNodeInTheNeighborhood(k, start_.key, clearing_dist_))
  { // unknown
    return true;
...
AstarPlanner::findPath(const octomap::point3d& start_point, const octomap::point3d& goal_point, std::shared_ptr<octomap::OcTree> planning_octree,
                       bool make_path_straight, bool apply_postprocessing, double planning_bbx_size_h, double planning_bbx_size_v,
                       double postprocessing_safe_dist, int postprocessing_max_iterations, bool postprocessing_horizontal_neighbors_only,
                       double postprocessing_z_tolerance, double postprocessing_path_length, int shortening_window_size, double shortening_dist,
                       bool apply_pruning, double pruning_dist, bool ignore_unknown_cells_near_start, double box_size_for_unknown_cells_replacement,
                       bool remove_obsolete_points, double obsolete_points_tolerance)
{

  if (make_path_straight && apply_postprocessing)
  {
    RCLCPP_WARN(m_node->get_logger(), "The path straightening cannot be applied together with the path postprocessing. ");
  }
  RCLCPP_INFO(m_node->get_logger(), "Findpath: Planning from [%.2f, %.2f, %.2f] to [%.2f, %.2f, %.2f]", start_point.x(), start_point.y(), start_point.z(),
  ...
  RCLCPP_INFO(m_node->get_logger(), "Tree resampling took %.2f s.", (m_node->now() - start).seconds());
  ...
  if (apply_postprocessing)
  {
    ...
    RCLCPP_INFO(m_node->get_logger(), "Path postprocessing took %.2f ms.", (m_node->now() - start).seconds() * 1000);

  } else if (make_path_straight)
  {
    waypoints = getStraightenWaypointPath(node_path, 0.2);
    RCLCPP_INFO(m_node->get_logger(), "Path straightening took %.2f s.", (m_node->now() - start).seconds());
  } else
  {
  ...
  RCLCPP_INFO(m_node->get_logger(), "----------------- Init path -------------------");
  for (size_t k = 0; k < node_path.size(); k++)
  {
    octomap::point3d p = planning_octree->keyToCoord(node_path[k].key);
    RCLCPP_INFO(m_node->get_logger(), "Node %lu: [%.2f, %.2f, %.2f]", k, p.x(), p.y(), p.z());
  }

  RCLCPP_INFO(m_node->get_logger(), "----------------- Final path -------------------");
  for (size_t k = 0; k < waypoints.size(); k++)
  {
    RCLCPP_INFO(m_node->get_logger(), "Node %lu: [%.2f, %.2f, %.2f]", k, waypoints[k].x(), waypoints[k].y(), waypoints[k].z());
  }
                                            ...
                                            double box_size_for_unknown_cells_replacement)
{
  ...
  if (!initialized_)
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Cannot start planning, planner not initialized. Returning empty path.");
    return waypoints;
  }
  ...
  RCLCPP_INFO(m_node->get_logger(), "Get node path start, resolution = %.2f", resolution_);

  octomap::OcTreeKey start_key = planning_octree_->coordToKey(start_point);
  RCLCPP_INFO(m_node->get_logger(), " astar planner ignore unknown cells near start = %d ", ignore_unknown_cells_near_start);
                                            ...
                                            bool ignore_unknown_cells_near_start, double box_size_for_unknown_cells_replacement)
{
  ...
  if (initial_waypoints.size() < 2)
  {
    RCLCPP_WARN(m_node->get_logger(),
                "[AstarPlanner]: Cannot start planning, vector of waypoints contains only %lu waypoints, at least 2 (start and goal) expected.",
  ...
  RCLCPP_INFO(m_node->get_logger(), " Get node path for multiple waypoints, resolution = %.2f", resolution_);
  for (size_t k = 1; k < initial_waypoints.size(); k++)
  {
    ...
    if (partial_waypoints.size() == 0)
    {
      if (waypoints.size() == 0)
      {
        RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Partial path to goal %lu not found proceeding to next point.", k);
        continue;
      } else
      {
        RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Partial path not found, returning found path.");
        return waypoints;
      }
    } else
    {
...
void AstarPlanner::replaceUnknownByFreeCells(const octomap::OcTreeKey& start_key, double box_size)
{
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), verbose_, "[AstarPlanner]: Replacing unknown cells by free in surrounding of point [%.2f, %.2f, %.2f].", p.x(),
                         p.y(), p.z());
  RCLCPP_INFO(m_node->get_logger(), "[debug]: unknown cells centers size before = %lu", unknown_cells_centers.size());
  ...
  RCLCPP_INFO(m_node->get_logger(), "[debug]: unknown cells centers size after = %lu", unknown_cells_centers_after.size());

  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Unknown cells replaced.");
}
//}
...
{
  RCLCPP_INFO(m_node->get_logger(), "[AstarPlanner]: Get node path start");
  std::vector<Node> waypoints;
  ...
  rclcpp::Time start_time = m_node->now();
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner] Start octomap to pointcloud");
  ...
  {
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), verbose_, "[AstarPlanner]: Start conversion");
    pcl::PointCloud<pcl::PointXYZ>::Ptr simulated_pointcloud = PCLMap::pclVectorToPointcloud(pcl_points);
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: init kd tree start");
    pcl_map_.initKDTreeSearch(simulated_pointcloud);
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: init kd tree end");
  }

  if (!checkValidityWithNeighborhood(goal_))
  {
    RCLCPP_WARN_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Goal destination unreachable.");
    Node secondary_goal_ = getValidNodeInNeighborhood(goal_);
    if (secondary_goal_.key.k[0] == 0)
    {
      RCLCPP_WARN_EXPRESSION(m_node->get_logger(), verbose_, "[AstarPlanner]: Secondary goal in the neighborhood not found. Destination unreachable.");
      if (!enable_planning_to_unreachable_goal_)
      {
        RCLCPP_WARN_EXPRESSION(m_node->get_logger(), verbose_, "[AstarPlanner]: Planning to unreachable goal not allowed. Returning empty path.");
        return waypoints;
      } else
      {
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Goal unreachable, but planning to unreachable goal allowed.");
      }
    } else
    {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Secondary goal found. Original goal [%d, %d, %d] replaced by [%d, %d, %d].",
      ...
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), verbose_,
                             "[AstarPlanner]: Secondary goal found. Original goal [%.2f, %.2f, %.2f] replaced by [%.2f, %.2f, %.2f].", goal_.pose.x(),
                             goal_.pose.y(), goal_.pose.z(), secondary_goal_.pose.x(), secondary_goal_.pose.y(), secondary_goal_.pose.z());
      goal_ = secondary_goal_;
    }
  }

  if (!checkValidityWithNeighborhood(start_) && safe_dist_prev_ < safe_dist_)
  { // prevents stuck due to increasing safe_dist
    safe_dist_ = safe_dist_prev_;
  }

  if (isNodeGoal(start_))
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Planner initialized at goal position. Returning empty plan.");
  ...
  if (waypoints_init.size() > 0)
  {
    ...
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Start position unfeasible. Generating path to nearest feasible node.");
  }

  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Add start into open list.");
  start_.f_cost = 0.0;
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Start key = [%d, %d, %d]", start_.key.k[0], start_.key.k[1], start_.key.k[2]);
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Goal key = [%d, %d, %d]", goal_.key.k[0], goal_.key.k[1], goal_.key.k[2]);

  while (!open_set.empty())
  {
    if (loop_counter % 100 == 0)
    {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Loop counter = %d, open list size = %lu, closed_list_size = %lu", loop_counter,
                             open_list.size(), closed_list.size());
      if ((m_node->now() - start_time).seconds() > (planning_timeout_ - map_conversion_time_))
      {
        RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Planning timeout reached.");
        break;
      }
    }
    ...
    {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Goal found");
      break;
    }
  ...
  RCLCPP_INFO(m_node->get_logger(), "[AstarPlanner debug]: Astar ended after %d iterations", loop_counter);
  ...
  RCLCPP_INFO(m_node->get_logger(), "[AstarPlanner debug]: Open set size %lu.", open_set.size());
  ...
  if (!isNodeGoal(current))
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Path not found, goal unreachable.");
    ...
    {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: current and nearest are not equal");
      current = nearest;
    }
  }

  last_found_goal_ = current;
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: start path reconstruction");
  current.pose = planning_octree_->keyToCoord(current.key);
  waypoints.push_back(current);
  int counter = 0;
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: path reconstruction %d, waypoint key = [%d, %d, %d]", counter,
                         waypoints[counter].key.k[0], waypoints[counter].key.k[1], waypoints[counter].key.k[2]);
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: reversing waypoints");
  std::reverse(waypoints.begin(), waypoints.end());
  waypoints_init.insert(waypoints_init.end(), waypoints.begin(), waypoints.end());
  /* stop_index         = 15; */
  /* start_node_next    = waypoints[0]; */
  rclcpp::Time end_time = m_node->now();
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: AstarPlanner: returning path of %lu waypoints", waypoints_init.size());
  RCLCPP_WARN_EXPRESSION(m_node->get_logger(), verbose_, "[AstarPlanner]: Path planning took %.3f ms", (end_time - start_time).seconds() * 1000.0);
...
std::vector<Node> AstarPlanner::getPathToNearestFeasibleNode(const Node& start)
{
  ...
  if (!checkValidityWithKDTree(start))
  { // start is not feasible, try to find closest feasible point
    RCLCPP_INFO(m_node->get_logger(), "gpnfn start node is not collision free ");
    ...
    std::vector<Node> waypoints;
    if (loop_counter < iter_limits - 1 || best.h_cost <= start_.h_cost + 1.1 * resolution_)
    { // solution found
      ...
      if (waypoints.size() > 0)
      {
        ...
        RCLCPP_INFO(m_node->get_logger(), "Replacing original start [%.2f, %.2f, %.2f] by start in the free space [%.2f, %.2f, %.2f].", start_.pose.x(),
                    start_.pose.y(), start_.pose.z(), waypoints_filtered.back().pose.x(), waypoints_filtered.back().pose.y(),
                    waypoints_filtered.back().pose.z());

        for (long unsigned int k = 0; k < waypoints_filtered.size(); k++)
        {
          RCLCPP_INFO(m_node->get_logger(), "escape path node [%ld] = [ %.2f, %.2f, %.2f] ", k, waypoints_filtered[k].pose.x(), waypoints_filtered[k].pose.y(),
                      waypoints_filtered[k].pose.z());
        }
      }
    }
  }

  RCLCPP_INFO(m_node->get_logger(), "Getting path to nearest feasible node took %.3f", (m_node->now() - start_time).seconds());
                                                          ...
                                                          double z_diff_tolerance, bool fix_goal_point, bool horizontal_neighbors_only, double timeout)
{
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: GetSafePath start");
  ...
  if (key_path.size() < 2)
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: getSafePath receives too short path (length = %lu)", key_path.size());
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: octomap to pointcloud start");
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: octomap to pointcloud ends");
  rclcpp::Time end_time = m_node->now();
  RCLCPP_WARN_EXPRESSION(m_node->get_logger(), verbose_, "Octomap to pointcloud took %.2f ms", (end_time - start_time).seconds() * 1000.0);
  if (pcl_points.size() > 0)
  {
    pcl::PointCloud<pcl::PointXYZ>::Ptr simulated_pointcloud = PCLMap::pclVectorToPointcloud(pcl_points);
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Map limits: x = [%d, %d], y = [%d, %d], z = [%d, %d]", map_limits[0], map_limits[1],
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: local_path_keys initialized");
  rclcpp::Duration last_iter_duration = rclcpp::Duration::from_seconds(0.0);
  for (int it = 0; it < max_iteration; it++)
  {

    if ((1 - timeout_percentage_for_filtering) * timeout - (m_node->now() - start_time).seconds() < last_iter_duration.seconds())
    {
      RCLCPP_INFO(m_node->get_logger(), "[AstarPlanner]: Path postprocessing timeout reached at iteration %d", it);
      break;
    }

    rclcpp::Time iter_start = m_node->now();

    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Start iteration %d", it);
    ...
    {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Local path key %d: [%d, %d, %d]", k, local_path_keys[k].k[0],
                             local_path_keys[k].k[1], local_path_keys[k].k[2]);
      /* last_waypoint                  = local_path_keys_next.back(); */
      is_current_key_already_in_plan = false;

      for (int j = local_path_keys_next.size() - 1; j > fmax(0, local_path_keys_next.size() - ceil(1.0 / resolution_)); j--)
      { // detection of similar nodes
      ...
      {
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Current key already in plan, continue to next key");
        continue;
      }
      ...
      if (pcl_map_.getDistanceFromNearestPoint(octomapKeyToPclPoint(local_path_keys[k])) > safe_dist)
      {
        ...
        {
          has_changed = true;
          RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Distance is safe and previous node is equal to current node. Nothing to add.");
        } else if (areKeysInNeighborhood(local_path_keys_next.back(), local_path_keys[k]))
        {
          ...
          RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Distance is safe. add [%d, %d, %d] to next path keys", local_path_keys[k].k[0],
                                 local_path_keys[k].k[1], local_path_keys[k].k[2]);
        } else
        {
          ...
          has_changed = true;
          RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_,
                                 "[AstarPlanner]: Distance is safe, but nodes are not in the neighborhood, add [%d, %d, %d], [%d, %d, %d] to next path keys",
      ...
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_,
                             "[AstarPlanner]: Dist is not safe, solving connection for [%d, %d, %d] and current [%d, %d, %d] with best neighbor [%d, %d, %d]",
                             local_path_keys_next.back().k[0], local_path_keys_next.back().k[1], local_path_keys_next.back().k[2], local_path_keys[k].k[0],
      ...
      {
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Current node is equal to previous node. Nothing to add.");
      ...
      } else if (areKeysInNeighborhood(tmp_key, local_path_keys_next.back()))
      {
        ...
        has_changed = true;
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_,
                               "[AstarPlanner]: Current node is in neighborhood of previous node. Adding the current node to local path.");
      ...
      } else
      {
        ...
        local_path_keys_next.push_back(tmp_key);
        has_changed = true;
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_ && (additional_waypoints.size() == 2),
        ...
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_ && (additional_waypoints.size() == 1),
    ...
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: First lpk size %lu = lpk next size = %lu", local_path_keys.size(),
    ...
    /* } */
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Second lpk size %lu = lpk next size = %lu", local_path_keys.size(),
                           local_path_keys_next.size());
    if (!has_changed)
    {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), verbose_, "[AstarPlanner]: No change detected -> safe path algorithm ended.");
      break;
    }
    RCLCPP_WARN_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: ------------------------------------------------------------------------");
    for (uint i = 0; i < local_path_keys.size(); i++)
    {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Safe path key %02d: [%d, %d, %d] ", i, local_path_keys[i].k[0],
                             local_path_keys[i].k[1], local_path_keys[i].k[2]);
    }
    RCLCPP_WARN_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: ------------------------------------------------------------------------");
    last_iter_duration = (m_node->now() - iter_start);
  }

  RCLCPP_WARN_EXPRESSION(m_node->get_logger(), debug_, "Get safe path took %.2f ms", (m_node->now() - start_time).seconds() * 1000.0);
  start_time = m_node->now();
  ...
  RCLCPP_WARN_EXPRESSION(m_node->get_logger(), debug_, "Safe path filtering took %.2f ms", (m_node->now() - start_time).seconds() * 1000.0);
...
std::vector<octomap::OcTreeKey> AstarPlanner::getZzFilteredPlan(const std::vector<octomap::OcTreeKey>& original_path, double tolerance)
{
  ...
  for (size_t k = 0; k < original_path.size(); k++)
  {
    ...
    {
      if (new_path.back().k[2] == original_path[k + m].k[2])
      { // FIXME: can be rewritten in more effective way
        partial_result.clear();
...
std::vector<octomap::OcTreeKey> AstarPlanner::getStraightenKeyPath(const std::vector<octomap::OcTreeKey>& key_path)
{
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Start key path straightening.");
  if (key_path.size() < 3)
  {
    straighten_path = key_path;
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Path too short to straighten, returning original path.");
  } else
  {
    ...
    for (uint i = 0; i < key_path.size() - 2; i++)
    {
      ...
      {
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Keys [%d, %d, %d] and [%d, %d, %d] are diagonal neighbors.", key_path[i].k[0],
                               key_path[i].k[1], key_path[i].k[2], key_path[i + 2].k[0], key_path[i + 2].k[1], key_path[i + 2].k[2]);
        straighten_path.push_back(key_path[i + 2]);
        i++; // skip i+1 key
      } else
      {
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Keys [%d, %d, %d] and [%d, %d, %d] are not diagonal neighbors.", key_path[i].k[0],
                               key_path[i].k[1], key_path[i].k[2], key_path[i + 2].k[0], key_path[i + 2].k[1], key_path[i + 2].k[2]);
...
std::vector<octomap::point3d> AstarPlanner::pruneWaypoints(std::vector<octomap::point3d>& waypoint_path, double pruning_dist)
{
  ...
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Nothing to prune. Returning original path.");
  ...
  for (size_t k = 1; k < waypoint_path.size() - 1; k++)
  {
    if (sqrt(pow(waypoint_path[k].x() - pruned_path.back().x(), 2) + pow(waypoint_path[k].y() - pruned_path.back().y(), 2)
             + pow(waypoint_path[k].z() - pruned_path.back().z(), 2))
        > pruning_dist)
    {
...
std::vector<octomap::point3d> AstarPlanner::getWaypointPathWithoutObsoletePoints(std::vector<octomap::point3d>& waypoint_path, double tolerance)
{
  ...
  if (waypoint_path.size() < 3)
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: No obsolete points. Returning original path.");
    return waypoint_path;
  }
...
std::vector<octomap::point3d> AstarPlanner::getStraightenWaypointPath(std::vector<Node>& node_path, double dist_step)
{
  ...
  if (node_path.size() < 2)
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: AstarPlanner: Empty node path received. Returning empty plan.");
    return waypoints;
  }
                                                              ...
                                                              double enabled_filtering_dist)
{
  ...
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Empty path received, returning empty filtered path.");
    return new_path;
  }
  for (uint k = 0; k < original_path.size(); k++)
  {
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Filtered path [%d] = [%d, %d, %d]", k, original_path[k].k[0], original_path[k].k[1],
                           original_path[k].k[2]);
  }
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Filtering: Last point of original_path = [%d, %d, %d] ",
                         original_path[original_path.size() - 1].k[0], original_path[original_path.size() - 1].k[1],
                         original_path[original_path.size() - 1].k[2]);
  new_path.push_back(original_path[0]);
  bool point_added = false;
  for (uint k = 0; k < original_path.size(); k++)
  {
  ...
  {
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Filtered path [%d] = [%d, %d, %d]", k, new_path[k].k[0], new_path[k].k[1],
                           new_path[k].k[2]);
  }
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Filtering: Last point of new_path = [%d, %d, %d] ", new_path[new_path.size() - 1].k[0],
...
std::vector<octomap::point3d> AstarPlanner::getWaypointPath(const std::vector<octomap::OcTreeKey>& key_path)
{
  ...
  if (key_path.empty())
  {
    RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: AstarPlanner: Empty key path received. Returning empty plan.");
    return waypoints;
  }
...
std::vector<octomap::OcTreeKey> AstarPlanner::getKeyVectorFromCoordinates(const std::vector<geometry_msgs::msg::Point>& pose_array)
{
...
std::pair<int, int> AstarPlanner::firstUnfeasibleNodeInPath(const std::vector<octomap::OcTreeKey>& key_waypoints,
                                                            const std::vector<geometry_msgs::msg::Point>& pose_array, int n_points_forward,
                                                            const octomap::point3d& current_pose, double safe_dist_for_replanning,
                                                            double critical_dist_for_replanning)
{
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: First unfeasible node in path start.");
  std::pair<int, int> result;
  result.first = -1;
  result.second = -1;
  int current_pose_idx = 0;
  for (uint k = 0; k < key_waypoints.size(); k++)
  { // FIXME add saving previously found starting index to avoid iterating over whole path
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: Current pose idx found.");
  ...
  if (pcl_points.size() > 0)
  {
    ...
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), true, "[AstarPlanner]: Map limits: x = [%d, %d], y = [%d, %d], z = [%d, %d]", map_limits[0], map_limits[1],
                           map_limits[2], map_limits[3], map_limits[4], map_limits[5]);
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: init kd tree start");
    pcl_map_.initKDTreeSearch(simulated_pointcloud);
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: init kd tree end");
...
std::vector<pcl::PointXYZ> AstarPlanner::octomapToPointcloud(const std::vector<int>& map_limits)
{
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: octomap to pointcloud start, x = [%d, %d], y = [%d, %d], z = [%d, %d]", map_limits[0],
  ...
  for (int x = map_limits[0]; x <= map_limits[1]; x++)
  {
    for (int y = map_limits[2]; y <= map_limits[3]; y++)
    {
      ...
      {
        /* RCLCPP_INFO(m_node->get_logger(),"[AstarPlanner]: x = %d, y = %d, z = %d", x, y, z); */
        pcl::PointXYZ point;
        octomap::OcTreeKey tmp_key;
        tmp_key.k[0] = x;
        tmp_key.k[1] = y;
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(), debug_, "[AstarPlanner]: octomap to pointcloud end");
  return output_pcl;
}
//}
...
std::vector<octomap::OcTreeKey> AstarPlanner::getAdditionalWaypoints(const octomap::OcTreeKey& k1, const octomap::OcTreeKey& k2)
{
  ...
  break;
  default: {
    RCLCPP_ERROR(m_node->get_logger(), "[AstarPlanner]: Number of different coords = %d outside expected range. ", nof_diff_coords);
  }
  }
...
std::vector<std::vector<octomap::OcTreeKey>> AstarPlanner::getPossibleWaypointsForTwoDiffCoord(const octomap::OcTreeKey& k1, const octomap::OcTreeKey& k2)
{
  ...
  if ((abs(k1.k[0] - k2.k[0]) + abs(k1.k[1] - k2.k[1]) + abs(k1.k[2] - k2.k[2])) == 2)
  {
    ...
    for (int m = 0; m < 3; m++)
    {
      ...
      {
        RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Something is crazy: diff in coords %d = %d", m, diff_in_coords[m]);
      }
    }
  } else
  {
  ...
  } else if (fabs(eucl_dist - sqrt(5)) < 1e-5)
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      case 2:
        ...
        break;
      default:
        RCLCPP_WARN(m_node->get_logger(), "[AstarPlanner]: Unexpected number in eucliden dist of keys.");
      }
...
octomap::OcTreeKey AstarPlanner::getBestNeighbor(const octomap::OcTreeKey& c, bool horizontal_neighbors_only)
{
  ...
  /* RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Get best neighbor: current dist = %.2f, max_dist = %.2f", current_dist, max_dist); */
...
std::vector<Node> AstarPlanner::getPossibleSuccessors(const octomap::OcTreeKey& parent, const octomap::OcTreeKey& current)
{
  ...
  if (move_length == 1)
  { // horizontal
    /* RCLCPP_INFO(m_node->get_logger(),"[AstarPlanner]: Move length == 1"); */
  ...
  { // 2D - diagonal
    /* RCLCPP_INFO(m_node->get_logger(),"[AstarPlanner]: Move length == 2"); */
  ...
  {
    /* RCLCPP_INFO(m_node->get_logger(),"[AstarPlanner]: Move length == 3"); */
    // cube
  ...
  /* RCLCPP_INFO(m_node->get_logger(),"[AstarPlanner]: Parent = [%d, %d, %d], current = [%d, %d, %d]", parent.k[0], parent.k[1], parent.k[2], current.k[0],
   * current.k[1], current.k[2]); */
  /* RCLCPP_WARN(m_node->get_logger(),"[AstarPlanner]: Size of successors = %lu", successors.size()); */
  /* for (size_t k = 0; k < successors.size(); k++) { */
  /*   RCLCPP_INFO(m_node->get_logger(),"[AstarPlanner]: Succesor %lu: [%d, %d, %d]", k, successors[k].key.k[0], successors[k].key.k[1],
...
{
  safe_dist_ = safe_dist;
  RCLCPP_INFO(m_node->get_logger(), "[AstarPlanner]: A* safe dist set to %.2f ", safe_dist_);
}
//}
...
void AstarPlanner::setAstarAdmissibility(const double astar_admissibility)
{
  ...
  RCLCPP_INFO(m_node->get_logger(), "[AstarPlanner]: A* admissibility set to %.2f ", astar_admissibility_);
}
//}
...
bool AstarPlanner::isNodeInTheNeighborhood(const octomap::OcTreeKey& n, const octomap::OcTreeKey& center, double dist)
{
  ...
  /* RCLCPP_INFO(m_node->get_logger(),"[AstarPlanner]: isNodeInTheNeighborhood: returning: %.2f ", (voxel_dist * resolution_) ); */
