...
std::pair<std::vector<octomap::point3d>, bool> AstarPlanner::findPath(
  ...
  RCLCPP_INFO(m_node->get_logger(),"Findpath: Planning from [%.2f, %.2f, %.2f] to [%.2f, %.2f, %.2f]", start_point.x(), start_point.y(),
...
std::vector<Node> AstarPlanner::getNodePath(const std::vector<octomap::point3d>& initial_waypoints, std::shared_ptr<octomap::OcTree> planning_octree,
  ...
  if (initial_waypoints.size() < 2) {
    RCLCPP_WARN(m_node->get_logger(),"[AstarPlanner]: Cannot start planning, vector of waypoints contains only %lu waypoints, at least 2 (start and goal) expected.",
...
void AstarPlanner::replaceUnknownByFreeCells(const octomap::OcTreeKey& start_key, double box_size) {
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(),verbose_, "[AstarPlanner]: Replacing unknown cells by free in surrounding of point [%.2f, %.2f, %.2f].", p.x(), p.y(), p.z());
...
std::vector<Node> AstarPlanner::getNodePath() {
  ...
  if (!checkValidityWithNeighborhood(goal_)) {
    ...
    if (secondary_goal_.key.k[0] == 0) {
      ...
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Secondary goal found. Original goal [%d, %d, %d] replaced by [%d, %d, %d].", goal_.key.k[0], goal_.key.k[1],
      ...
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(),verbose_, "[AstarPlanner]: Secondary goal found. Original goal [%.2f, %.2f, %.2f] replaced by [%.2f, %.2f, %.2f].", goal_.pose.x(),
  ...
  while (!open_set.empty()) {
    if (loop_counter % 100 == 0) {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Loop counter = %d, open list size = %lu, closed_list_size = %lu", loop_counter, open_list.size(),
                    closed_list.size());
  ...
  int counter = 0;
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: path reconstruction %d, waypoint key = [%d, %d, %d]", counter, waypoints[counter].key.k[0],
  ...
  if (!checkValidityWithKDTree(start)) {  // start is not feasible, try to find closest feasible point
    ...
    if (loop_counter < iter_limits - 1 || best.h_cost <= start_.h_cost + 1.1 * resolution_) {  // solution found
      ...
      if (waypoints.size() > 0) {
        ...
        RCLCPP_INFO(m_node->get_logger(),"Replacing original start [%.2f, %.2f, %.2f] by start in the free space [%.2f, %.2f, %.2f].",
        ...
        for (int k = 0; k < waypoints_filtered.size(); k++) {
          RCLCPP_INFO(m_node->get_logger(),"escape path node [%d] = [ %.2f, %.2f, %.2f] ", k, waypoints_filtered[k].pose.x(),
...
std::vector<octomap::OcTreeKey> AstarPlanner::getSafePath(const std::vector<octomap::OcTreeKey>& key_path, double safe_dist, int max_iteration,
  ...
  if (pcl_points.size() > 0) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr simulated_pointcloud = PCLMap::pclVectorToPointcloud(pcl_points);
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Map limits: x = [%d, %d], y = [%d, %d], z = [%d, %d]", map_limits[0], map_limits[1], map_limits[2], map_limits[3],
  ...
  for (int it = 0; it < max_iteration; it++) {
    ...
    for (uint k = 1; k < local_path_keys.size(); k++) {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Local path key %d: [%d, %d, %d]", k, local_path_keys[k].k[0], local_path_keys[k].k[1], local_path_keys[k].k[2]);
      ...
      if (pcl_map_.getDistanceFromNearestPoint(octomapKeyToPclPoint(local_path_keys[k])) > safe_dist) {
        ...
        if (areKeysEqual(local_path_keys_next.back(), local_path_keys[k])) {
          ...
          RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Distance is safe. add [%d, %d, %d] to next path keys", local_path_keys[k].k[0], local_path_keys[k].k[1],
          ...
          has_changed = true;
          RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Distance is safe, but nodes are not in the neighborhood, add [%d, %d, %d], [%d, %d, %d] to next path keys",
      ...
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Dist is not safe, solving connection for [%d, %d, %d] and current [%d, %d, %d] with best neighbor [%d, %d, %d]",
                    local_path_keys_next.back().k[0], local_path_keys_next.back().k[1], local_path_keys_next.back().k[2], local_path_keys[k].k[0],
      ...
      if (areKeysEqual(tmp_key, local_path_keys_next.back())) {
        ...
        has_changed = true;
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Current node is in neighborhood of previous node. Adding the current node to local path.");
    ...
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: First lpk size %lu = lpk next size = %lu", local_path_keys.size(), local_path_keys_next.size());
    ...
    /* } */
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Second lpk size %lu = lpk next size = %lu", local_path_keys.size(), local_path_keys_next.size());
    if (!has_changed) {
    ...
    for (uint i = 0; i < local_path_keys.size(); i++) {
      RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Safe path key %02d: [%d, %d, %d] ", i, local_path_keys[i].k[0], local_path_keys[i].k[1], local_path_keys[i].k[2]);
    }
    RCLCPP_WARN_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: ------------------------------------------------------------------------");
  ...
  if (key_path.size() < 3) {
    ...
    for (uint i = 0; i < key_path.size() - 2; i++) {
      if (areKeysDiagonalNeighbors(key_path[i], key_path[i + 2])) {
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Keys [%d, %d, %d] and [%d, %d, %d] are diagonal neighbors.", key_path[i].k[0], key_path[i].k[1],
                      key_path[i].k[2], key_path[i + 2].k[0], key_path[i + 2].k[1], key_path[i + 2].k[2]);
        straighten_path.push_back(key_path[i + 2]);
        i++;  // skip i+1 key
      } else {
        RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Keys [%d, %d, %d] and [%d, %d, %d] are not diagonal neighbors.", key_path[i].k[0], key_path[i].k[1],
                      key_path[i].k[2], key_path[i + 2].k[0], key_path[i + 2].k[1], key_path[i + 2].k[2]);
...
std::vector<octomap::OcTreeKey> AstarPlanner::getFilteredPlan(const std::vector<octomap::OcTreeKey>& original_path, int window_size,
  ...
  for (uint k = 0; k < original_path.size(); k++) {
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Filtered path [%d] = [%d, %d, %d]", k, original_path[k].k[0], original_path[k].k[1], original_path[k].k[2]);
  }
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Filtering: Last point of original_path = [%d, %d, %d] ", original_path[original_path.size() - 1].k[0],
  ...
  for (uint k = 0; k < new_path.size(); k++) {
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: Filtered path [%d] = [%d, %d, %d]", k, new_path[k].k[0], new_path[k].k[1], new_path[k].k[2]);
  }
...
std::pair<int, int> AstarPlanner::firstUnfeasibleNodeInPath(const std::vector<octomap::OcTreeKey>&   key_waypoints,
  ...
  if (pcl_points.size() > 0) {
    ...
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(),true, "[AstarPlanner]: Map limits: x = [%d, %d], y = [%d, %d], z = [%d, %d]", map_limits[0], map_limits[1], map_limits[2], map_limits[3],
                  map_limits[4], map_limits[5]);
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: init kd tree start");
    pcl_map_.initKDTreeSearch(simulated_pointcloud);
    RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: init kd tree end");
...
std::vector<pcl::PointXYZ> AstarPlanner::octomapToPointcloud(const std::vector<int>& map_limits) {
  ...
  RCLCPP_INFO_EXPRESSION(m_node->get_logger(),debug_, "[AstarPlanner]: octomap to pointcloud start, x = [%d, %d], y = [%d, %d], z = [%d, %d]", map_limits[0], map_limits[1], map_limits[2],
